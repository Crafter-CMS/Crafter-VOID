...
export default async function PunishmentTypePage({ 
  ...
  const punishmentService = serverPunishmentService();
  let punishments: any[] = [];
  ...
  try {
    if (WEBSITE_ID) {
      ...
      pagination = response.pagination || { page: 1, limit: 10, total: 0, totalPages: 0, hasNext: false, hasPrev: false };
    }
  } catch (error) {
                        ...
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            <Avatar className="w-8 h-8">
                              <AvatarImage src={`https://mc-heads.net/avatar/${punishment.uuid}`} />
                              <AvatarFallback className="bg-gradient-to-br from-teal-400 to-blue-600">
                                {punishment.name.charAt(0).toUpperCase()}
                              </AvatarFallback>
                            </Avatar>
                            <div>
                              <div className="font-medium text-red-500">{punishment.name}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            <Avatar className="w-8 h-8">
                              <AvatarImage src={`https://mc-heads.net/avatar/${punishment.operator}`} />
